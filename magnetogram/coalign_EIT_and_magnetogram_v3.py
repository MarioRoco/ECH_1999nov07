...
# Date and time: change the format
from datetime import datetime
dt_eit = datetime.strptime(header_eit['DATE-OBS'], "%Y-%m-%dT%H:%M:%S.%fZ")
date_eit_formatted = dt_eit.strftime("%Y/%m/%d - %H:%M:%S")


### Plot: EIT full Sun, coordinates instead of pixels
fig, ax = plt.subplots(figsize=(9.5, 9))
img = ax.pcolormesh(x_eit_arcsec, y_eit_arcsec, data_eit_inverse, norm=LogNorm(), cmap='Greys_r', shading='auto')
cax = fig.add_axes([0.87, 0.1, 0.025, 0.82]) # [left, bottom, width, height]
cbar = fig.colorbar(img, ax=ax, cax=cax, pad=0.01)
cbar.set_label('DN/s', fontsize=16, labelpad=1)
fig.subplots_adjust(left=0.12, right=0.86, bottom=0.1, top=0.92, wspace=0.2, hspace=0.2)
ax.axis('equal')
ax.set_title(f'EIT - {header_eit["WAVELNTH"]}, {date_eit_formatted} UTC', fontsize=20)
...
#Date: change the format
from datetime import datetime
dt_mag = datetime.strptime(header_mag['UTDATE'], "%m/%d/%y")
date_mag_formatted = dt_mag.strftime("%Y/%m/%d")


### Plot: Magnetogram full Sun, coordinates instead of pixels
v_max = 100.
fig, ax = plt.subplots(figsize=(9.5, 9))
img = ax.pcolormesh(x_mag_arcsec, y_mag_arcsec, data_mag_inverse, vmin=-v_max, vmax=v_max, cmap='Greys_r')
cax = fig.add_axes([0.87, 0.1, 0.025, 0.82]) # [left, bottom, width, height]
cbar = fig.colorbar(img, ax=ax, cax=cax, pad=0.01)
cbar.set_label('Magnetic field (Gauss)', fontsize=16, labelpad=1)
fig.subplots_adjust(left=0.12, right=0.86, bottom=0.1, top=0.92, wspace=0.2, hspace=0.2)
ax.axis('equal')
ax.set_title(f'Magnetogram {date_mag_formatted} - {header_mag["UTSTART"]} UT', fontsize=20)
ax.set_xlabel('Helioprojective longitude (arcsec)', fontsize=17)
ax.set_ylabel('Helioprojective latitude (arcsec)', fontsize=17)
# Contours
from skimage import measure
