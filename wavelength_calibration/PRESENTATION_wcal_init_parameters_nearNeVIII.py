...
row = 234
idx_gaussian_to_show_dic = {'1':'all', '2':[0], '3':[1], '4':[1], '5':[0,1]}
intervals_to_show = ['2','4','5']
...
color_list = ['blue', 'green', 'red', 'orange', 'magenta', 'olive', 'brown', 'lime', 'blue', 'red', 'green', 'orange', 'magenta', 'olive', 'brown', 'lime']
...
vmin_spimg, vmax_spimg = 1e-2, 5e0
fig_spimg, ax_spimg = plt.subplots(nrows=1, ncols=1, figsize=(12., 6))
img_spimg = ax_spimg.imshow(raster_average[6:323+1], cmap='Greys', aspect='auto', norm=LogNorm(vmin=vmin_spimg, vmax=vmax_spimg), extent=[-0.5, 512.5,323.5,5.5])
cax_spimg = fig_spimg.add_axes([0.91, 0.11, 0.02, 0.77])  # [left, bottom, width, height]
cbar_spimg = fig_spimg.colorbar(img_spimg, cax=cax_spimg)
cbar_spimg.set_label(f'Av. Radiance [W/sr/m^2/\u212B]', fontsize=16)
ax_spimg.set_title(f'SOHO/SUMER average of all spectral images', fontsize=18) 
ax_spimg.set_xlabel('Wavelength direction (pixels)', color='black', fontsize=16)
ax_spimg.set_ylabel('Spatial direction (pixels)', color='black', fontsize=16)
ax_spimg.axhline(y=row, linewidth=2., color='orange', label=f'row {row}')
ax_spimg.legend(fontsize=16)
...
def pixels_to_wavelength(pixels): return np.interp(pixels, x_pixels, x_wavelength)
def wavelength_to_pixels(wavelength): return np.interp(wavelength, x_wavelength, x_pixels)
#secax = ax.secondary_xaxis('top', functions=(pixels_to_wavelength, wavelength_to_pixels))
...
ax.set_title(f"Fitting multiple gaussians to the spectrum of row {row}", fontsize=18)
ax.set_xlabel('Wavelength direction (pixels)', fontsize=15)
ax.set_ylabel(r'Radiance (W m^-2 sr^-1 nm^-1)', fontsize=15)
ax.errorbar(x=x_pixels, y=y_radiance, yerr=y_unc_radiance, linewidth=1.2, color='black', label='SUMER data', zorder=1)
...
for i, interval_str in enumerate(list(idx_interval_dic)):
    ...
    for j, n_gaussian_to_analyze in enumerate(range(n_gaussians)):
        ...
        ax.plot(x_fit_singlegauss, y_fit_singlegauss, color=color_list[j], linestyle='-', label=f'Single gaussian. Mean: {np.round(mean_fit,3)} +- {np.round(mean_unc_fit,3)} px', zorder=1) 
...
##########################################################
# Real SUMER spectrum: polynomial fit to the lowest N points


#lowest N data points
N = 10
idx_mins = np.argsort(y_radiance)[:N] 
x_wavelength_mins = x_wavelength[idx_mins]
y_radiance_mins = y_radiance[idx_mins]
y_unc_radiance_mins = y_unc_radiance[idx_mins]

# Fit a polynomial to the lowest data points
popt_sumer, pcov_sumer = curve_fit(quadratic_function, x_wavelength_mins, y_radiance_mins, sigma=y_unc_radiance_mins, absolute_sigma=True, p0=[0, 0, 1]) #p0 os the initial guess, in this case: p0=[a,b,c], being a, b, c the coefficients of f(x)=a*x^2+b*x+c
perr_sumer = np.sqrt(np.diag(pcov_sumer))  # Standard errors of the parameters #TODO: check if this is the correct way to calculate the uncertainties
a_sumer, b_sumer, c_sumer = popt_sumer
sa_sumer, sb_sumer, sc_sumer = perr_sumer
...
#for i, interval_str in enumerate(list(idx_interval_dic)):
for i, interval_str in enumerate(intervals_to_show):
    print('Interval number =', interval_str)
    init_parameters = init_parameters_dic[interval_str]
    idx_interval = idx_interval_dic[interval_str]
    ...
    ax.errorbar(x=x_data, y=y_data, yerr=y_unc_data, linewidth=0.7, color=color_list[i], label='Interval ' + interval_str) 
    ax.plot(x_fit, y_fit, color=color_list[i], linestyle='-') #Fit
    ax.axvspan(x_data[0], x_data[-1], color=color_list[i], alpha=0.15)
...
fig, ax = plt.subplots(figsize=(12, 5))
#ax.set_title(f"Fitting multiple gaussians to the spectrum of row {row}", fontsize=18)
ax.set_title(f"Different multigaussian fits to the spectrum of row {row}", fontsize=18)
ax.set_xlabel('Wavelength direction (pixels)', fontsize=15)
ax.set_ylabel(r'Radiance (W m$^{-2}$ sr$^{-1}$ nm$^{-1}$)', fontsize=15)
ax.errorbar(x=x_pixels, y=y_radiance, yerr=y_unc_radiance, linewidth=1.2, color='black', label='SUMER data', zorder=1)


x_shift_meanfit = [5, 5, -5, 5]
k=0
#for i, interval_str in enumerate(list(idx_interval_dic)):
for i, interval_str in enumerate(intervals_to_show):
    print('Interval number =', interval_str)
    ...
    n_gaussians = (len(init_parameters)-1)//3
    for j, n_gaussian_to_analyze in enumerate(range(n_gaussians)):
        if idx_gaussian_to_show_dic[interval_str]=='all' or n_gaussian_to_analyze in idx_gaussian_to_show_dic[interval_str]:
            amplitude_fit = popt[3*n_gaussian_to_analyze+1]
            mean_fit = popt[3*n_gaussian_to_analyze+2]
            fwhm_fit = popt[3*n_gaussian_to_analyze+3]
            mean_unc_fit = perr[3*n_gaussian_to_analyze+2]
            ...
            ax.plot(x_fit_singlegauss, y_fit_singlegauss, color=color_list[i], linestyle='-', label=f'Single gaussian. Mean: {np.round(mean_fit,3)} +- {np.round(mean_unc_fit,3)} px', zorder=1)
            ax.axvline(x=mean_fit, color=color_list[i])
            ax.axvspan(mean_fit-mean_unc_fit, mean_fit+mean_unc_fit, alpha=0.1, color=color_list[i]) #bckg uncertainty
            
            # Add mean and uncertainty text
            text_y_position = max(y_fit_singlegauss) * 1.4  # Slightly above the Gaussian peak
            ax.text(mean_fit+0.4+x_shift_meanfit[k], text_y_position, f'{mean_fit:.2f} ± {mean_unc_fit:.2f} px', 
            rotation=90, va='bottom', ha='center', fontsize=10, color=color_list[i])
...
ax.errorbar(x=x_data, y=y_data, linewidth=0, marker='^', markersize=8, color='blue', label='Data')
...
for i, interval_str in enumerate(intervals_to_show):
    fig, ax = plt.subplots(figsize=(12, 5))
    ax.set_title(f"Multigaussian fit", fontsize=18)
    ax.set_xlabel('Wavelength direction (pixels)', fontsize=15)
    ax.set_ylabel(r'Radiance (W m$^{-2}$ sr$^{-1}$ nm$^{-1}$)', fontsize=15)
    ax.errorbar(x=x_pixels, y=y_radiance, yerr=y_unc_radiance, linewidth=1., color='black', label='SUMER data')
    #ax.errorbar(x=x_data, y=y_data, yerr=y_unc_data, linewidth=1., color='black')#, label='Interval ' + interval_str) 
    ...
    ax.plot(x_fit, y_fit, color=color_list[i], linestyle='-', linewidth=2., label='Multigaussian fit') #Fit
    ...
    for j, n_gaussian_to_analyze in enumerate(range(n_gaussians)):
        ...
        ax.plot(x_fit_singlegauss, y_fit_singlegauss, color=color_list[j], linestyle='-', linewidth=1., label=f'Single gaussian. Mean: {np.round(mean_fit,3)} +- {np.round(mean_unc_fit,3)} px', zorder=1)
        ax.axvline(x=mean_fit, color=color_list[j])
        ax.axvspan(mean_fit-mean_unc_fit, mean_fit+mean_unc_fit, alpha=0.1, color=color_list[j]) #bckg uncertainty
    ...
    for j, n_gaussian_to_analyze in enumerate(range(n_gaussians)):
        if idx_gaussian_to_show_dic[interval_str]=='all' or n_gaussian_to_analyze in idx_gaussian_to_show_dic[interval_str]:
            ...
            ax.plot(x_fit_singlegauss, y_fit_singlegauss, color=color_list[i], linestyle='--', linewidth=1.3)#, label=f'Single gaussian. Mean: {np.round(mean_fit,3)} +- {np.round(mean_unc_fit,3)} px', zorder=1)
            ax.axvline(x=mean_fit, color=color_list[i])
            ax.axvspan(mean_fit-mean_unc_fit, mean_fit+mean_unc_fit, alpha=0.1, color=color_list[i]) #bckg uncertainty
            
            # Add mean and uncertainty text
            text_y_position = max(y_fit_singlegauss) * 1.1  # Slightly above the Gaussian peak
            ax.text(mean_fit+0.4, text_y_position, f'{mean_fit:.2f} ± {mean_unc_fit:.2f} px', 
            rotation=90, va='bottom', ha='center', fontsize=13, color=color_list[i])
            
    ax.plot([],[], color=color_list[i], linestyle='--', linewidth=2., label=f'Single gaussian fit', zorder=1)
    ax.set_xlim([idx_interval_dic[interval_str][0]-2, idx_interval_dic[interval_str][1]+2])
