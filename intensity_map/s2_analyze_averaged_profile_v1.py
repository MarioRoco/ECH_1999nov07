...
filename_averaged_profile = 'average_profile__0_0__4_0__max_of_sumer_NeVIII.npz'
...
if sun_region=='qqr_a':
    ...
    init_parameters_corrected = [bckg_fit_corrected, #[background, amplitude1, mean1, FWHM1, amplitude2, mean2, FWHM2,...]
                   ...
                   0.325-bckg_fit_corrected, 0.0, 50.,
...
filename_profile = '../data/data_modified/'+filename_averaged_profile
...
x_uncorrected = lam_sumer_cropNeVIII[4:-5]
y_uncorrected = rad_sumer_cropNeVIII[4:-5]
y_unc_uncorrected = erad_sumer_cropNeVIII[4:-5]
...
ax[0].set_ylabel(r'Spectral radiance (W m$^{-2}$ sr$^{-1}$''\u212B'r'$^{-1}$)', fontsize=15)
...
ax.errorbar(x=vkms_doppler(lamb=x_corrected, lamb_0=lamb_0) ,y=y_corrected, yerr=y_unc_corrected, color=color_sumer_corrected, marker='o', linewidth=0, elinewidth=1., label='SUMER corrected')
ax.plot(x_fit_corrected, y_fit_corrected, color='green', linestyle='-', label='Fit', zorder=1) 
#ax[0].plot(vkms_doppler(lamb=x_fit_corrected_singlegauss, lamb_0=lamb_0), y_fit_corrected_singlegauss, color='magenta', linestyle='-', label='Individual gaussian', zorder=1)
...
for n_gauss in range(N_gaussians):
    ...
    x_fit_corrected_singlegauss = np.linspace(mean_fit-sigma_fit*3., mean_fit+sigma_fit*3., 200)
    y_fit_corrected_singlegauss = gaussian_function_with_background(x=x_fit_corrected_singlegauss, bckg=bckg_fit, amplitude=amplitude_fit, mean=mean_fit, fwhm=fwhm_fit)
    ax.plot(x_fit_corrected_singlegauss, y_fit_corrected_singlegauss, color=color_i, linestyle=':')#, label='Individual gaussians')
ax.plot([], [], color='grey', linestyle=':', label='Individual gaussians')

ax.axvline(x=0, color='black', linestyle='--', label='Ne VIII/2')
ax.axvspan(-v_unc_0, v_unc_0, color='grey', alpha=0.15)
ax.set_title(f'SUMER spectrum lowest 4%, multigaussian fit', fontsize=18)
#ax[0].set_title(f'SUMER {range_percentage}%, corrected', fontsize=18)
