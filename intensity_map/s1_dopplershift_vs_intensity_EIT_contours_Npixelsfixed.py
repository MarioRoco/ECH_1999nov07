...
bound_i = np.min(data_eit_crop_corrected)-0.01
...
while bound_i<np.max(data_eit_crop_corrected):
    
    lower_bound_eit = bound_i
    rowscols_croplat_eit = pixels_higher_intensity_than_a_value(arr_2d=data_eit_crop_corrected, bound_=bound_i, N_pixels=N_pixels_bin)
    row_max, col_max = rowscols_croplat_eit[-1] #address of the pixel with maximum intensity of this bin
    bound_i = data_eit_crop_corrected[row_max, col_max] #maximum intensity of this bin
    upper_bound_eit = bound_i
    ...
    rowscols_croplat_eit = np.array(rowscols_croplat_eit)
